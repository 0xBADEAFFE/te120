...
{
	DECLARE_CLASS( C_WeaponPhysConcussion, C_BaseHLCombatWeapon );
public:
	C_WeaponPhysConcussion( void );
...
STUB_WEAPON_CLASS_IMPLEMENT( weapon_physconcussion, C_WeaponPhysConcussion );
...
// Constructor
//-----------------------------------------------------------------------------
C_WeaponPhysConcussion::C_WeaponPhysConcussion( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void C_WeaponPhysConcussion::OnDataChanged( DataUpdateType_t updateType )
{
	...
	if ( SetupEmitter() )
	{
		...
		if ( m_bIsCurrentlyUpgrading )
		{
			...
			for ( i = 0; i < set->numhitboxes; ++i )
			{
				...
				ComputeRenderInfoConc( pBox, *hitboxbones[pBox->bone], &vecAbsOrigin, &xvec, &yvec );
...
void C_WeaponPhysConcussion::ClientThink( void )
{
